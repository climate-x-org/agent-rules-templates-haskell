...
## Base Rules
- Use `stylish-haskell` to maintain a consistent code style.
- Use `hlint` to catch common anti-patterns and suggest improvements.
- Use a single `cabal.project` at repository root with local packages under `packages/`.
- Use an explicit `default-language` in Cabal (`GHC2024` when supported, otherwise `GHC2021`).
- Keep warnings enabled (`-Wall`) and treat warnings as errors in CI (`-Werror`).
- Avoid partial functions (`head`, `tail`, `fromJust`); model failure with `Maybe`/`Either`.
- Use `Either` for recoverable errors and reserve exceptions for unrecoverable failures.
- Keep pure domain logic separate from effectful `IO` boundaries.
- Use explicit or qualified imports to avoid ambiguity and clarify dependencies.
- Be deliberate about strictness (`!` fields or `seq`) on long-lived data to avoid space leaks.
- Commit `cabal.project.freeze` to support reproducible builds across developer and CI environments.
- Use Hspec for unit/integration tests and QuickCheck for property-based tests.
- Write Haddock documentation for exported modules, types, and functions.
...
- Run `stylish-haskell` (or formatter equivalent) when formatting updates are intended.
- Run `hlint .`.
- Run `cabal build all`.
- Run `cabal test all`.
